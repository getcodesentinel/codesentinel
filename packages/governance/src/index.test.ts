...
const makeAnalysis = (score: number, hotspot: string, cycleCount = 0): AnalyzeSummary => ({
  structural: {
    ...
    files: [{ id: hotspot, relativePath: hotspot, directDependencies: [], fanIn: 0, fanOut: 0, depth: 0 }],
  ...
  risk: {
    ...
    fileScores: [{ file: hotspot, score, normalizedScore: score / 100, factors: { structural: 1, evolution: 0, external: 0 } }],
...
describe("governance gates", () => {
  it("evaluates diff gates deterministically", () => {
    const baseline = createSnapshot({ analysis: makeAnalysis(30, "src/a.ts", 0), generatedAt: "2026-03-01T00:00:00.000Z" });
    const current = createSnapshot({ analysis: makeAnalysis(70, "src/b.ts", 1), generatedAt: "2026-03-01T00:01:00.000Z" });
  ...
  it("throws when diff gate is configured without baseline", () => {
    const current = createSnapshot({ analysis: makeAnalysis(40, "src/a.ts"), generatedAt: "2026-03-01T00:00:00.000Z" });
    expect(() =>
      evaluateGates({
        current,
        gateConfig: {
