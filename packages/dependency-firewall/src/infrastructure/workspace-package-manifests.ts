...
import type { WorkspaceKind } from "@codesentinel/core";
import type { WorkspaceDependencyManifest } from "../domain/types.js";
...
export const loadWorkspaceDependencyManifests = (
  projectRoot: string,
  ...
  const manifestsByPath = new Map<string, WorkspaceDependencyManifest>();
  for (const pattern of includePatterns) {
    for (const absoluteWorkspacePath of expandPattern(absoluteRoot, pattern)) {
      ...
      manifestsByPath.set(relativePath, {
        name: packageJson?.name ?? basename(absoluteWorkspacePath),
        path: relativePath,
        kind: inferWorkspaceKind(relativePath),
        directDependencies: parsePackageJson(rawPackageJson),
      });
    }
  }

  return [...manifestsByPath.values()].sort((a, b) => a.path.localeCompare(b.path));
};
